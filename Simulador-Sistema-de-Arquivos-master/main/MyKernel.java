 ...
 * @author Matheus Henrique Moreno da Silva
 */
...
public class MyKernel implements Kernel {
	...
	public String cp(String parameters) {
		...
		if (!auxParametros[0].contains(".txt")) {
			...
			hd.setBloco(
					Abstracao.alteraFilho(hd.blocoToString(buscaParametro(auxParametros[1]).getPonteiro()),
							Binario.intToBinaryString(novoEndereco), hd),
					buscaParametro(auxParametros[1]).getPonteiro());

			// hd.setBloco(Abstracao.diretorioToBinario(novoDiretorio, hd),
			// novoDiretorio.getPonteiro());
			// fim da implementacao do aluno
		}
		return result;
	}
	...
	public String mv(String parameters) {
		// variavel result deverah conter o que vai ser impresso na tela apos comando
		// dobuscaParametro(auxParametros[1]).getPonteiro()
		// usuário
		String result = "";
		...
		if (!auxParametros[0].contains(".txt")) {
			...
			int posicao = buscaParametro(auxParametros[0].substring(0, auxParametros[0].length() - nomeA.length()))
					.getPai()
					.getPonteiro();
			...
			if (!parameters.contains(".txt")) {
				...
				for (int i = 0; i < filhos.size(); i++) {
					...
					if (nome.equals(nomeA)) {
						...
						int pos = Binario
								.binaryStringToInt(Abstracao.getEnderecoBloco(hd.blocoToString(filhos.get(i))));
	...
	public String info() {
		...
		// nome do aluno
		String name = "Matheus Henrique Moreno da Silva";
		// numero de matricula
		String registration = "202011020005";
