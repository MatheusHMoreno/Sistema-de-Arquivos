...
public class Binario {
	...
	/**
	 * Transforma um valor inteiro em um String contendo 0's e 1's equivalente ao
	 * valor inteiro em binario
	 * 
	 * @param value
	 *               O valor inteiro a ser convertido.
	 * @param length
	 *               Numero de posicoes para o String binario.
	...
	public static String binarioToString(String binario) {
		...
		String x = binario.toString().replaceAll(" ", "");
		char c, n;
		for (int i = 0; i < x.toString().length(); i = i + 8) {
			if (i + 8 <= x.toString().length()) {
				String valor = x.toString().substring(i, i + 8);
				c = (char) Integer.parseInt(valor, 2);
	...
	public static String booleanToString(boolean[] bloco) {
		StringBuilder str = new StringBuilder();
